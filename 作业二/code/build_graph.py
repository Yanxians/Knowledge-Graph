...
class GraphBuilder:
    def __init__(self, conn):
        self.conn = conn
        self.data_path =  "C:\\Users\\yanran\\Downloads\\QASystemOnMedicalKG-main\\作业二\\data\\disease.csv"
    def read_file(self):
        """
        读取文件，获得实体，实体关系
        :return:
...
CALL db.createVertexLabel(
    ...
    'name', 'string', false,
    'age', 'string', true,
    'infection', 'string', true,
    'insurance', 'string', true,
    'checklist', 'string', true,
    'treatment', 'string', true,
    'period', 'string', true,
    'rate', 'string', true,
    'money', 'string', true
)
...
"""
    ...
    schema_complications = """
CALL db.createVertexLabel(
    'Complication', 'name', 
    'name', 'string', false)
"""
    schemas_v.append(schema_complications)
...
"""
    ...
    builder.conn.query("CALL db.createEdgeLabel('LOCATED_IN', '[[\"Disease\",\"Part\"]]', 'name', 'string', true)", db='kanye')
    builder.conn.query("CALL db.createEdgeLabel('HAS_COMPLICATION', '[[\"Disease\",\"Complication\"]]', 'name', 'string', true)", db='kanye')
